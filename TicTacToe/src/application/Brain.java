...
public class Brain {
	  ...
	  public Brain (EvolvingPlayer player, int layerCount, int inputNeuronCount, int outputNeuronCount, int hiddenLayerNeuronCount) {
	    this.player = player;
	    this.inputNeuronCount = inputNeuronCount;
	    this.outputNeuronCount = outputNeuronCount;
	    this.inputs = new double [inputNeuronCount];
	  ...
	  public void reward(Game game) {
	    ...
	    for (int i = 0; i<layers.get(layers.size()-1).neurons.get(decision).weights.length-1; i++) {
	      layers.get(layers.size()-1).neurons.get(decision).weights[i] += 1.0f;
	    }
	    // Other Weights
	    ...
	    for (int i = layers.size()-2; i>0; i--) { 
	      for (int j = 0; j<layers.get(i).neurons.get(maxWeightIndex).weights.length-1; j++) {
	        ...
	        layers.get(i).neurons.get(maxWeightIndex).weights[j] += 0.1f*i;
	      }
	  ...
	  public void punish(Game game) {
	    ...
	    /*
	    System.out.println("ActivationLevels");
	    ...
	    for (int i = layers.size()-1; i>0; i--) {
	      for (int j = 0; i<layers.get(i).neurons.size(); j++) {
	        if (layers.get(i).neurons.get(j).activationLevel>
	      }
	    }
	    */
	  }
